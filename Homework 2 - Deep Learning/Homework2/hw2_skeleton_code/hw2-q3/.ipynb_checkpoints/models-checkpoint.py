...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        # Embed the current target token
        embedded = self.embedding(tgt_step)  # (batch_size, hidden_size)
        embedded = self.dropout(embedded).unsqueeze(1)  # Add time dimension: (batch_size, 1, hidden_size)

        # Pass through the LSTM
        output, dec_state = self.lstm(embedded, dec_state)  # output: (batch_size, 1, hidden_size)
