...
def nucleus_sampling(logits, p=0.8):
    ...
    Returns:
        next_token: index of the next predicted token. Shape: (1,)
    """
    # Step 1: Transform logits into probabilities using softmax
    probs = torch.softmax(logits, dim=-1)  # Shape: (vocab_size,)

    # Step 2: Sort the probabilities in descending order and get sorted indices
    sorted_probs, sorted_indices = torch.sort(probs, descending=True)

    # Step 3: Compute the cumulative probabilities
    cumulative_probs = torch.cumsum(sorted_probs, dim=-1)  # Shape: (vocab_size,)

    # Step 4: Find the cutoff where cumulative probability >= p
    cutoff_index = torch.searchsorted(cumulative_probs, p, right=True).item()

    # Step 5: Select the top-p vocabulary (indices where cumulative_prob <= p)
    top_p_indices = sorted_indices[:cutoff_index + 1]  # Tokens in the top-p set
    top_p_probs = sorted_probs[:cutoff_index + 1]     # Corresponding probabilities

    # Step 6: Normalize the probabilities of the selected tokens to sum to 1
    top_p_probs /= top_p_probs.sum()

    # Step 7: Sample from the top-p probabilities
    next_token = torch.multinomial(top_p_probs, num_samples=1)  # Sample one token
    next_token = top_p_indices[next_token]  # Map back to original vocabulary indices
